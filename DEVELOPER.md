...
blocks/
└── [block-name]/
    ├── style.css       # Edit this for frontend changes
    ├── editor.css      # Edit this for editor-only changes
    └── build/          # Don't edit - auto-generated
```
...
.price-highlight {
  color: #4a90e2;  /* ❌ Not in our palette */
}
```
...
.card {
  padding: 27px;     /* ❌ Not a standard value */
  margin-bottom: 19px;  /* ❌ Use 20px instead */
}
```
...
.section-title {
  font-family: 'Comic Sans';  /* ❌ Never do this */
}
```
...
<h2><?php echo $title; ?></h2>  /* ❌ XSS vulnerability */
```
...
<span style="color: #2563eb;">  /* ❌ Use CSS or attributes */
```
